...
public class SerialInputOutputManager implements Runnable {
    ...
    private final StringBuilder readBufferAccumulator = new StringBuilder();
    ...
    private void step() throws IOException {
        ...
        if (len > 0) {
            ...
            synchronized (readBufferAccumulator){
                readBufferAccumulator.append(new String(buffer, 0, len));

                int endIndex = readBufferAccumulator.indexOf("\r");
                if (endIndex == -1) {
                    endIndex = readBufferAccumulator.indexOf("\n");
                }

                while (endIndex != -1) {
                    // Extract the complete message up to the delimiter
                    String completeMessage = readBufferAccumulator.substring(0, endIndex).trim();
                    readBufferAccumulator.delete(0, endIndex + 1); // Remove processed message

                    // Notify the listener with the complete message
                    final Listener listener = getListener();
                    if (listener != null) {
                        listener.onNewData(completeMessage.getBytes());
                    }

                    // Check for more complete messages
                    endIndex = readBufferAccumulator.indexOf("\r");
                    if (endIndex == -1) {
                        endIndex = readBufferAccumulator.indexOf("\n");
                    }
                }
